...
from pandas import *
import fuckit, keyboard, time, sys, os, datetime
...
class Dungeon:
    def __init__(self):
        ...
        for count in range(self.count_chemists):
            ...
            while not found:
                ...
                if not self.Matrix[x][y][0] == "#":
                    self.Matrix[x][y] = [self.symbol_chemists, 0, [people().chemist()]]
                    found = True
        ...
        while not found:
            ...
            if self.Matrix[x][y][0] != "#":
                """
                to maintain at least a solid 14 distance between goal and player
                """
    ...
    def check(self):
        self.playerontile_type = self.Matrix[self.current_loc[0]][self.current_loc[1]][2][0]
        #print(self.Matrix[self.current_loc[0]][self.current_loc[1]][2])
        self.playerontile_inv = self.Matrix[self.current_loc[0]][self.current_loc[1]][2][2] # inv list
        ...
        if self.playerontile_type == self.symbol_chemists:
            thechemist = self.Matrix[self.current_loc[0]][self.current_loc[1]][2][2][0]
    ...
    def print_tile_inv(self):
        ...
        if len(self.playerontile_inv) == 1:
            ...
            constructor = '{} and {}'.format(', '.join(self.playerontile_inv[:-1]), self.playerontile_inv[-1])
    ...
    def printleaderboard(self):
        ...
        for i, element in enumerate(self.leaderboardList):
            ...
            print("{the_prefix}{index}: [{name}]\n\tTime: {time}\n\tMoves: {moves}\n\tDate and Time: {datetime}".format(the_prefix = self.prefix_text, index = str(i+1), name = element["name"], time = element["time"], moves = element["moves"], datetime = element["datetime"]))
    ...
    def gameOver(self, how):
        if how == "exit":
            ...
            print("You have failed to escape the [DUNGEON], you'll do better next time.")
            sys.exit()
    def traderscreen(self, typeofperson, obj):
        ...
        self.traders = obj
        os.system("cls")
        print("{0} - {1}".format(color(obj.name, "magenta"), color(typeofperson.title(), "green")))
        for index, item in enumerate(self.traders.stuff):
            print("{0}: {1} {2}".format(index+1, item["name"].title(), color("[{}]".format(item["cost"]), "magenta")))
                ...
                if pressed.isdigit():
                    ...
                    if self.coins  >= self.traders.stuff[pressed-1]["cost"]:
                        if len(self.inventory) == self.maxinventory:
                            ...
                            print("You have bought the {}".format(self.traders.stuff[pressed-1]["name"].title()))
                            self.inventory.append(self.traders.stuff[pressed-1])
                            self.printinventory()
                    else:
                        print("You do not have enough money to buy the {}.".format(self.traders.stuff[pressed-1]["name"].title()))
                    time.sleep(0.3)
                if pressed == "e":
                    os.system("cls")
                    self.printmap()
    ...
    def attack(self, enemy_type): # He protecc he attacc he also like to snacc
        if enemy_type == "orc":
            ...
            self.Matrix[self.current_loc[0]][self.current_loc[1]][2] = [self.symbol_empty, 1]
            print(color("The {0} drops {1} coins.".format(self.orcs.name, self.orcs.coins), "yellow"))
    ...
    def enemyhit(self, enemy):
        if not random.randint(1, 100) < enemy.percentage:
            self.current_attack_damage = enemy.attack_base + random.randint(enemy.attack_range[0], enemy.attack_range[1])
    ...
    def hit(self, enemy):
        if not random.randint(1, 100) < self.equipped["chance-of-hit"]:
            self.current_attack_damage = self.equipped["base-attack"] + random.randint(self.equipped["random-attack"][0], self.equipped["random-attack"][1])
            if self.current_attack_damage == self.equipped["base-attack"] + self.equipped["random-attack"][1]:
                # Max Damage
                print(self.equipped["text-critical"].format(self.equipped["name"]))
            else:
                print(self.equipped["text-normal"].format(enemy.name, self.equipped["name"]))
    ...
    def printinventory(self):
        print(color("Inventory ({0} / {1})".format(len(self.inventory), self.maxinventory), "green"))
        if len(self.inventory) == 0:
            print("You have nothing in your inventory!")
        else:
            for index, item in enumerate(self.inventory):
                print("{0}: {1}\n\t{2}".format(index+1, item["name"].title(), item["description"]))
        ...
        if direction == "e":
            if self.current_loc[1] < (self.w-1):
                if self.Matrix[self.current_loc[0]][self.current_loc[1]+1][0] != "#":
                    self.Matrix[self.current_loc[0]][self.current_loc[1]] = self.Matrix[self.current_loc[0]][self.current_loc[1]][2]
                    self.Matrix[self.current_loc[0]][self.current_loc[1]+1] = ["@", 1, self.Matrix[self.current_loc[0]][self.current_loc[1]+1]]
        ...
        if direction == "n":
            if self.current_loc[0] > 0:
                if self.Matrix[self.current_loc[0]-1][self.current_loc[1]][0] != "#":
                    self.Matrix[self.current_loc[0]][self.current_loc[1]] = self.Matrix[self.current_loc[0]][self.current_loc[1]][2]
                    self.Matrix[self.current_loc[0]-1][self.current_loc[1]] = ["@", 1, self.Matrix[self.current_loc[0]-1][self.current_loc[1]]]
    ...
    def printmap(self):
        print("{0} {1} {2} {3} {4} {5} {6}".format(color("[Dungeon]", "red"), color("Move: {}".format(self.moves), "green"), color("Health: ({0} / {1})".format(self.health, self.maxhealth), "cyan"), color("XP: {}".format(self.xp), "magenta"), color("Coins: {}".format(self.coins), "yellow"), color("Inventory ({0} / {1})".format(len(self.inventory), self.maxinventory), "green"), color("Time: {}".format(time.time()-self.time), "green")))
        self.Map = []
        index = -1
...
""")
    ...
    def player(self, state):
        ...
        if state == 1:
            ...
            self.name = input("Hello adventure, what is your name? (Enter for random name)\n> ")
